...
const { FiMessageSquare, FiMail, FiPhone, FiUser, FiClock, FiAlertCircle, FiCheckCircle, FiSend, FiPaperclip, FiMoreHorizontal, FiSearch, FiFilter, FiTag, FiFlag, FiArchive, FiReply, FiForward, FiTrash, FiStar, FiUsers, FiActivity, FiTrendingUp, FiEye, FiRefreshCw, FiSettings, FiEdit, FiCheck, FiX, FiZap, FiArrowLeft, FiPlus, FiMinus, FiMaximize2, FiMinimize2, FiCopy, FiDownload, FiUpload, FiBold, FiItalic, FiUnderline, FiList, FiLink, FiImage, FiSmile, FiAtSign } = FiIcons;
...
const AdminMessages = () => {
  ...
  const [tickets, setTickets] = useState([
    ...
    {
      id: 'TKT-003',
      subject: 'Bug Report: Mobile Menu Issue',
      user: {
        name: 'Emma Davis',
      ...
      messages: [
        {
          id: 1,
          sender: 'user',
          message: 'The navigation menu hamburger icon doesn\'t open on mobile devices. Tested on iPhone 12 and Samsung Galaxy.',
        ...
        },
        {
          id: 2,
          sender: 'admin',
          message: 'Thank you for reporting this! I\'ve reproduced the issue and it\'s been fixed in version 2.1.1. Please update your template.',
          timestamp: new Date(Date.now() - 86400000),
          type: 'text'
        },
        {
          id: 3,
          sender: 'user',
          message: 'Perfect! Updated and it\'s working now. Thanks for the quick fix!',
  ...
  const handleImageUpload = async (files) => {
    ...
    for (const file of files) {
      if (file.type.startsWith('image/')) {
        ...
        if (activeTab === 'chat') {
          setNewMessage(prev => prev + imageText);
        } else if (activeTab === 'tickets') {
          setNewTicketReply(prev => prev + imageText);
  ...
  const IconButton = ({ 
    ...
    onClick, 
    title, 
    className = "neu-button p-2 bg-gray-600 text-gray-300 hover:bg-gray-500 relative",
    children,
    active = false
  }) => (
    <div className="dropdown-container relative">
      <button
        onClick={onClick}
        className={`${className} ${active ? 'bg-blue-600 text-white' : ''} transition-all duration-200`}
        title={title}
      >
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-900 to-blue-900 p-4 lg:p-6">
      <div className="max-w-7xl mx-auto">
        ...
        <motion.div
          ...
          className="neu-card p-4 mb-6 bg-gray-800 border border-blue-500/30"
        >
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4">
              <button
                onClick={() => navigate('/admin/dashboard')}
                className="neu-button p-2 bg-gray-700 text-gray-300 hover:bg-gray-600"
              >
                <SafeIcon icon={FiArrowLeft} className="w-5 h-5" />
              </button>
              <div className="neu-button p-2 bg-blue-600 text-white">
                <SafeIcon icon={FiMessageSquare} className="w-5 h-5" />
              </div>
              <div>
                <h1 className="text-xl font-bold text-white">
                  Communication <span className="text-blue-400">Center</span>
                </h1>
            ...
            <div className="flex items-center gap-2">
              ...
              <button
                ...
                className={`neu-button px-3 py-1 text-sm ${showAIAssistant ? 'bg-purple-600 text-white' : 'bg-gray-700 text-gray-300'}`}
              >
        ...
        {/* Main Interface with Tabs and Content */}
        <div className="neu-card p-6 bg-gray-800 h-[calc(100vh-200px)]">
          {/* Compact Tabs with Stats */}
          <div className="flex gap-2 mb-4">
            {[
              ...
              <button
                ...
                className={`neu-button px-4 py-2 hover:scale-105 transition-transform relative ${
          ...
          <div className="flex gap-4 mb-4">
            <div className="relative flex-1">
              ...
              <input
                ...
                className="w-full neu-input pl-10 pr-4 py-2 bg-gray-700 text-white text-sm"
          ...
          {uploading && (
            <div className="fixed top-4 right-4 neu-card p-4 bg-gray-800 border border-blue-500/30 z-50">
          ...
          <div className="h-[calc(100%-120px)]">
            ...
            {activeTab === 'chat' && (
              <div className="grid lg:grid-cols-4 gap-4 h-full">
                {/* Chat List */}
                <div className="neu-card-inset p-3 bg-gray-700/30 overflow-y-auto">
                      ...
                      >
                        <div className="flex items-center gap-2">
                          <div className="relative">
                            ...
                            />
                            <div className={`absolute -bottom-1 -right-1 w-2 h-2 rounded-full border border-gray-800 ${
                          ...
                          <div className="flex-1 min-w-0">
                            <div className="flex items-center justify-between">
                              <div className="font-medium truncate text-sm">{chat.user.name}</div>
                              {chat.unread > 0 && (
                                <div className="bg-red-500 text-white text-xs rounded-full w-4 h-4 flex items-center justify-center">
                                  {chat.unread}
                                </div>
                            ...
                            <div className="text-xs opacity-75 truncate">{chat.lastMessage}</div>
                          </div>
                        </div>
                      </motion.div>
                    ))}
                ...
                {/* Chat Messages Area */}
                <div className="lg:col-span-3 neu-card-inset bg-gray-700/30 flex flex-col">
                  {selectedChat ? (
                    <>
                      {/* Chat Header */}
                      <div className="p-3 border-b border-gray-600 flex items-center justify-between">
                        <div className="flex items-center gap-2">
                          ...
                          />
                          <div>
                            <div className="text-white font-medium text-sm">{selectedChat.user.name}</div>
                            <div className="text-gray-400 text-xs">{selectedChat.user.email}</div>
                          </div>
                        </div>
                      ...
                      {/* AI Assistant Panel */}
                      {showAIAssistant && (
                        <div className="p-3 bg-purple-900/30 border-t border-gray-600">
                          <div className="flex items-center justify-between mb-2">
                            <h4 className="text-white font-medium text-sm flex items-center gap-1">
                              <SafeIcon icon={FiZap} className="w-3 h-3 text-purple-400" />
                              AI Assistant
                            </h4>
                            <button
                              onClick={() => generateAISuggestions(newMessage, 'response')}
                              disabled={isGeneratingAI}
                              className="neu-button px-2 py-1 bg-purple-600 text-white text-xs"
                            >
                            ...
                            <div className="space-y-1">
                              {aiSuggestions.map((suggestion, index) => (
                                <button
                                  ...
                                  className="w-full text-left p-2 bg-gray-600/30 rounded text-gray-300 hover:bg-gray-600/50 text-xs"
                                >
                      ...
                      {/* Enhanced Message Input with Functional Icons */}
                      <div className="p-3 border-t border-gray-600 space-y-3">
                        ...
                        <div>
                          <textarea
                            ...
                            className="w-full neu-input p-3 bg-gray-700 text-white text-sm h-24 resize-none"
                        ...
                        <div className="flex items-center justify-between">
                          <div className="flex items-center gap-2">
                            ...
                            >
                              {showEmojiPicker && (
                                <div className="absolute bottom-full left-0 mb-2 neu-card p-4 bg-gray-800 border border-gray-600 rounded-lg shadow-xl z-50">
                                  <div className="text-white font-medium text-sm mb-3">Quick Emojis</div>
                          ...
                          <button
                            ...
                            className="neu-button px-6 py-2 bg-blue-600 text-white hover:bg-blue-700 disabled:opacity-50 disabled:cursor-not-allowed"
                          >
                            <div className="flex items-center gap-2">
            ...
            {/* Support Tickets Interface - Enhanced with Functional Icons */}
            {activeTab === 'tickets' && (
              <div className="grid lg:grid-cols-4 gap-4 h-full">
                {/* Ticket List */}
                <div className="neu-card-inset p-3 bg-gray-700/30 overflow-y-auto">
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          <div className="flex-1 min-w-0">
                            <div className="flex items-center gap-1 mb-1">
                              ...
                              <span className={`px-1 py-0.5 rounded text-xs font-medium ${getStatusColor(ticket.status)}`}>
                                {ticket.status}
                              </span>
                              <SafeIcon 
                                icon={FiFlag} 
                                className={`w-2 h-2 ${getPriorityColor(ticket.priority)}`} 
                              />
                            </div>
                ...
                <div className="lg:col-span-3 neu-card-inset bg-gray-700/30 flex flex-col">
                  {selectedTicket ? (
                    <>
                      ...
                      <div className="p-3 border-b border-gray-600">
                        <div className="flex items-center justify-between mb-2">
                          <div className="flex items-center gap-2">
                            ...
                            />
                            <div>
                              <div className="text-white font-medium text-sm">{selectedTicket.user.name}</div>
                              <div className="text-gray-400 text-xs">{selectedTicket.user.email}</div>
                            </div>
                          </div>
                          <div className="flex items-center gap-2">
                            ...
                            <span className={`px-2 py-1 rounded-full text-xs font-medium ${
                              ...
                              selectedTicket.priority === 'medium' ? 'bg-yellow-100 text-yellow-800' : 'bg-green-100 text-green-800'
                            }`}>
                              {selectedTicket.priority} priority
                            </span>
                          </div>
                        </div>
                        <div className="text-white font-semibold text-sm">{selectedTicket.subject}</div>
                        <div className="text-gray-400 text-xs flex items-center gap-4 mt-1">
                      ...
                      {/* Enhanced Ticket Actions & Reply with Functional Icons */}
                      <div className="p-3 border-t border-gray-600 space-y-3">
                        {/* Action Buttons */}
                        <div className="flex gap-2">
                          {selectedTicket.status !== 'resolved' ? (
                            <button
                              ...
                              className="neu-button px-3 py-1 bg-green-600 text-white text-xs"
                            >
                              <div className="flex items-center gap-1">
                            ...
                            <button
                              ...
                              className="neu-button px-3 py-1 bg-yellow-600 text-white text-xs"
                            >
                              <div className="flex items-center gap-1">
                                <SafeIcon icon={FiReply} className="w-3 h-3" />
                                Reopen
                              </div>
                            </button>
                          )}
                          <button className="neu-button px-3 py-1 bg-gray-600 text-white text-xs">
                            <div className="flex items-center gap-1">
                              <SafeIcon icon={FiFlag} className="w-3 h-3" />
                              Priority
                            </div>
                          </button>
                          <button className="neu-button px-3 py-1 bg-purple-600 text-white text-xs">
                            <div className="flex items-center gap-1">
                        ...
                        <div className="flex items-center justify-between">
                          <div className="flex items-center gap-2">
                            ...
                            {/* Priority Selector */}
                            <select className="neu-input p-1 bg-gray-700 text-white text-xs">
                    ...
                    <div className="flex-1 flex items-center justify-center">
                      <div className="text-center">
                        <SafeIcon icon={FiFlag} className="w-12 h-12 text-gray-400 mx-auto mb-3" />
                        <h3 className="text-lg font-semibold text-white mb-1">Select a Ticket</h3>
                        <p className="text-gray-400 text-sm">Choose a support ticket to start responding</p>
                      </div>
                    </div>
                  )}
                </div>
            ...
            {/* Email Management Interface with Enhanced Icons */}
            {activeTab === 'email' && (
                ...
                <div className="neu-card-inset p-3 bg-gray-700/30 overflow-y-auto">
                  <div className="flex items-center justify-between mb-3">
                    ...
                    <button
                      ...
                      className="neu-button px-2 py-1 bg-blue-600 text-white text-xs"
                    >
                  ...
                  <div className="space-y-2">
                    {filteredEmails.map((email) => (
                      <motion.div
                        ...
                        initial={{ opacity: 0, x: -20 }}
                        animate={{ opacity: 1, x: 0 }}
                        onClick={() => {setSelectedEmail(email); setIsReplying(false);}}
                        className={`p-2 rounded-lg cursor-pointer transition-all ${
                          selectedEmail?.id === email.id 
                            ? 'bg-blue-600 text-white' 
                            : `bg-gray-600/30 text-gray-300 hover:bg-gray-600/50 ${!email.read ? 'border-l-2 border-blue-400' : ''}`
                        }`}
                      >
                        <div className="flex items-start justify-between">
                          <div className="flex-1 min-w-0">
                            <div className="flex items-center gap-1 mb-1">
                              ...
                              {email.starred && <SafeIcon icon={FiStar} className="w-3 h-3 text-yellow-400" />}
                              {email.priority === 'high' && <SafeIcon icon={FiFlag} className="w-3 h-3 text-red-400" />}
                            </div>
                ...
                <div className="lg:col-span-2 neu-card-inset bg-gray-700/30 flex flex-col">
                  {selectedEmail === 'compose' ? (
                    <>
                      {/* Compose Header */}
                      <div className="p-3 border-b border-gray-600 flex items-center justify-between">
                        <h3 className="text-white font-medium">Compose Email</h3>
                        <button
                          onClick={() => setSelectedEmail(null)}
                          className="neu-button p-1 bg-gray-600 text-gray-300"
                        >
                      ...
                      <div className="flex-1 p-3 overflow-y-auto space-y-3">
                        <div>
                          <input
                            ...
                            className="w-full neu-input p-2 bg-gray-700 text-white text-sm"
                        ...
                        {emailCompose.attachments.length > 0 && (
                          <div className="neu-card-inset p-3 bg-gray-600/30">
                            <div className="text-white text-sm font-medium mb-2">Attachments:</div>
                            <div className="space-y-2">
                        ...
                        <div>
                          <textarea
                            ...
                            className="w-full neu-input p-3 bg-gray-700 text-white text-sm h-40 resize-none"
                            placeholder="Type your message... (Press Enter for new line)"
                          />
                        </div>
                        ...
                        <div className="flex items-center gap-2">
                          <IconButton
                            ...
                            onClick={() => {
                              ...
                              // Override to handle email attachments
                              const originalHandler = fileInputRef.current?.onchange;
                              fileInputRef.current.onchange = (e) => {
                                handleFileUpload(e.target.files, 'email');
                      ...
                      <div className="p-3 border-t border-gray-600">
                        <div className="flex gap-2 justify-end">
                          <button
                            onClick={() => setSelectedEmail(null)}
                            className="neu-button px-3 py-2 bg-gray-600 text-white text-sm"
                          >
                            Cancel
                          </button>
                          <button
                            ...
                            className="neu-button px-3 py-2 bg-blue-600 text-white text-sm disabled:opacity-50 disabled:cursor-not-allowed"
                          >
                            <div className="flex items-center gap-1">
                    ...
                    <>
                      ...
                      <div className="p-3 border-b border-gray-600">
                        <div className="flex items-start justify-between mb-2">
                          <div>
                            <h3 className="text-white font-medium mb-1">{selectedEmail.subject}</h3>
                          ...
                          <div className="flex items-center gap-1">
                            <button 
                              ...
                              className="neu-button p-1 bg-blue-600 text-white"
                              title="Reply"
                            >
                              <SafeIcon icon={FiReply} className="w-3 h-3" />
                            </button>
                            <button 
                              ...
                              className="neu-button p-1 bg-green-600 text-white"
                              title="Forward"
                            >
                              <SafeIcon icon={FiForward} className="w-3 h-3" />
                            </button>
                            <button 
                              onClick={() => toggleStar(selectedEmail.id)}
                              className={`neu-button p-1 ${selectedEmail.starred ? 'bg-yellow-500 text-white' : 'bg-gray-600 text-gray-300'}`}
                              title="Star"
                            >
                              <SafeIcon icon={FiStar} className="w-3 h-3" />
                            </button>
                            <button 
                              className="neu-button p-1 bg-red-600 text-white"
                              title="Delete"
                            >
                      ...
                      <div className="flex-1 p-3 overflow-y-auto">
                        ...
                        {selectedEmail.attachments.length > 0 && (
                          <div className="mt-4 pt-3 border-t border-gray-600">
                            <h4 className="text-white font-medium mb-2 text-sm">Attachments:</h4>
                            <div className="space-y-1">
                              {selectedEmail.attachments.map((attachment, index) => (
                                <div key={index} className="flex items-center gap-2 p-2 bg-gray-600/30 rounded text-sm">
                                  <SafeIcon icon={FiPaperclip} className="w-3 h-3 text-gray-400" />
                                  <span className="text-gray-300">{attachment.name}</span>
                                  <span className="text-gray-500 text-xs">({attachment.size})</span>
                                  <button className="ml-auto neu-button p-1 bg-blue-600 text-white">
                    ...
                    <>
                      {/* Reply Header */}
                      <div className="p-3 border-b border-gray-600 flex items-center justify-between">
                        <h3 className="text-white font-medium">Reply</h3>
                        <button
                    ...
                    <div className="flex-1 flex items-center justify-center">
                      <div className="text-center">
                        ...
                        <h3 className="text-lg font-semibold text-white mb-1">Select an Email</h3>
                        <p className="text-gray-400 text-sm">Choose an email to view or compose a new one</p>
                      </div>
                    </div>
                  )}
                </div>
